...
Разговорные ответы генерируются отдельным вызовом и допустимы только для приветствия, эмпатии, уточнения и мета-вопросов; дешёвого шаблонного пути для приветствий нет. Любой практический совет, план, норма, количество или рекомендация по психологии, БЖЖ, тренировкам, питанию либо БАДам становится knowledge-ответом и создаётся только по найденному контексту. Для обычной рекомендации бот отвечает native reply на последнее сообщение текущего пакета и добавляет кнопку `Источник`; URL строится из публичного username группы либо официального `t.me/c` для supergroup. Если URL недоступен, используется native reply на источник. Цитирование проверяется как точная UTF-16-совместимая подстрока источника и имеет безопасный fallback.
