...
Помощник работает как поиск по этой утверждённой базе, а не как универсальный собеседник. Содержательные ответы ограничены психологией, БЖЖ, тренировками на массу и для бойцов, марафоном «приведи себя в форму», витаминами/БАДами и планами питания; они создаются только при найденном контексте. Запросы вне этой области получают краткий отказ, а при отсутствии подходящего материала — сообщение, что его нет в базе канала. Для найденных первоисточников worker может вернуть только проверяемый Telegram-native reply/quote.
